 ...
 */
package org.palading.camphor.common.spi;
...
/**
 * camphor spi loader extendClass cache, the key is ClassType name,the inner key is spikey. one spikey can be more than
 * one implementation class
 * 
 * @author palading_cr
 * @title CamphorExtendClassLoader
 * @project camphor
 */
public class CamphorExtendClassLoader implements CamphorExtendClassLoaderFactory {

    private static ConcurrentHashMap<String, Map<String, List<?>>> extendClassInstances = new ConcurrentHashMap<>();

    private static CamphorExtendClassLoader camphorExtendClassLoaderInstance = new CamphorExtendClassLoader();

    public static CamphorExtendClassLoader getCamphorExtendClassLoaderInstance() {
        return camphorExtendClassLoaderInstance;
    }
    ...
    public <T> List<T> getExtendClassInstanceList(Class<T> type, String spiKey) {
        try {
            if (null == type) {
                throw new Exception("CamphorExtendClassLoader[getExtendClassInstanceList] loadbalance type is null");
            }
            if (StringUtils.isEmpty(spiKey)) {
                throw new Exception("CamphorExtendClassLoader[getExtendClassInstanceList] spiKey is null");
            }
    ...
    private synchronized <T> List<T> doCreateInstance(Class<T> type, ClassLoader classLoader, String spiKey) throws Exception {
        Map<String, List<String>> preparedInstance = CamphorClassFactoryLoader.loadExtendClassFactories(type, classLoader);
